...
When a user mentions "Wide Research" or references this file, load these instructions. You act as the primary orchestrator coordinating reusable multi-agent workflows using VS Code Copilot CLI. Tasks may involve web research, code retrieval, API sampling, data cleaning, etc. Execute flexibly while staying safe and compliant. **Important: use VS Code Copilot CLI's default settings unless the user explicitly authorizes changes.**
...
2. Spawn a fresh Copilot CLI process for each sub-goal with the appropriate permissions (default to restricted tool access, only enable broader access when required).
...
0. **Pre-run planning & reconnaissance (mandatory)**
   ...
   - When public indices (tag pages, API lists, etc.) exist, cache them via minimal restricted fetches and count entries. Otherwise, run light-touch desk research (news feeds, search, existing datasets) to surface representative items, capturing concrete evidence such as source URLs, timestamps, and key takeaways.
...
1. **Initialization**
  ...
  - Use VS Code Copilot CLI's default settings; only modify if the user explicitly authorizes changes.
...
3. **Generate the scheduler script**
    - Build a rerunnable driver script (e.g., `run_children.sh`) that:
      ...
      - Invokes `copilot` per subtask with recommended flags:
        - use `--allow-tool` to grant necessary permissions (e.g., `--allow-tool 'shell(git)'`, `--allow-tool 'file-write'`); be selective and only enable what's required for each subtask
        - make it explicit in the prompt that web search tools (preferably Tavily if available via MCP) are mandatory for external data; fall back to `curl`/`wget` only when absolutely necessary, and disable plan tools or human-in-the-loop pauses
        - avoid model overrides unless the user insists; use default Copilot settings
        - write outputs under predictable paths such as `child_outputs/<id>.md`
      - Embed in every child prompt: (a) a worked example of the desired report layout and (b) the known-good `copilot` call pattern. Clarify that the showcased command is a single-run example (no parallelism) meant to illustrate parameters, and remind the child to run `copilot --help` first when it needs to reason about CLI capabilities.
      - size timeouts to the subtask: start with 5 minutes for lightweight work, allow up to 15 minutes for heavier runs, and wrap with `timeout` at the script level. If the first 5-minute window expires, reassess (split, tune, or extend) before retrying; hitting 15 minutes signals the prompt/flow needs debugging.
      ...
      - Capture exit codes while streaming logs into the run directory via `stdbuf -oL -eL copilot -p "..." ... | tee logs/<id>.log` so operators can `tail -f` progress in real time.
      - Remember that `copilot` uses `-p` or `--prompt` for the prompt text; send output through pipes to write files, check the correct `PIPESTATUS` index when chaining commands, and keep the canonical pattern handy:
...
4. **Design child prompts**
   - Dynamically generate prompt templates that include:
     ...
     - explicit tool constraints (prioritize web search tools like Tavily if available via MCP; prefer MCP tools over native network commands, `wget`/`curl`, plan tool usage, or pauses waiting for humans)
     - reminders to keep search/extract iterations within 10 rounds—plan efficiently and stop when information is sufficient
...
- Escalate permissions only when justified; avoid `--allow-all-tools` unless absolutely necessary and always review what tools are being granted access.
...
- **Prefer MCP tools for networking**: avoid direct commands like `wget`/`curl` when possible; all external data should flow through MCP tools (prefer tavily_search / tavily_extract) if available.
...
- **Search provider preference**: before launching search-heavy subtasks, inspect available MCP servers or tools. If Tavily is available via MCP, route all search requests through Tavily; fall back to other search methods only when Tavily is absent.
